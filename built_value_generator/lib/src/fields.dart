...
/// returned.
BuiltList<FieldElement2> collectFields(InterfaceElement2 element) =>
...
/// If a field is overridden then just the closest (overriding) field is
/// returned.
BuiltList<FieldElement2> collectFieldsForType(InterfaceType type) {
  ...
  var fieldSet = LinkedHashSet<FieldElement2>(
  ...
  return BuiltList<FieldElement2>.build((b) => b
    ..addAll(fieldSet)
    ..where((field) =>
        type
            .lookUpGetter3(
...
BuiltList<FieldElement2> _fieldElementsForType(InterfaceType type) {
  var result = ListBuilder<FieldElement2>();
  for (var accessor in type.getters) {
    result.add(accessor.variable3 as FieldElement2);
  }
  return result.build();
}
